...
class SubjectItemAdapter extends RecyclerView.Adapter<SubjectItemAdapter.SubjectItemViewHolder> {
    ...
    public SubjectItemAdapter(Context context, List<Subject> subjects, int semesterDividerIndex) {
        ...
        StreamSupport.stream(subjects).filter(subject -> subject.getModules() != null).forEach((Subject subject) -> {
            data.addAll(subject.getModules());
        });
        Collections.sort(data, (a, b) -> {
            try {
                return new SimpleDateFormat("dd.MM.yy").parse(a.getDate()).compareTo(new SimpleDateFormat("dd.MM.yy").parse(b.getDate()));
...
//        }
        ...
        holder.subjectDate.setText(current.getDate());
        String currentSubjectName = "";
        for (Subject subject : subjects) {
            if (subject.getModules() != null) {
                for (Module module : subject.getModules()) {
                    if (module.equals(current)) {
                        currentSubjectName = subject.getName();
                    }
                }
            }
        }
        holder.subjectName.setText(currentSubjectName);
